...
store.getORderInforbyId(router.query.orderID as string);
</script>
<template>
  <div class="xtx-pay-page">
    <div class="container">
      ...
      <div class="pay-result">
        ...
        <p>
          支付方式：<span>{{
            store.orderInfo.result.payType === 1 ? "支付宝" : "微信"
          }}</span>
        </p>
        <p>
          <span>支付金额:¥{{ store.orderInfo.result.payMoney }}</span>
        </p>
        <div class="btn">
          <XtxButton
            as="link"
            :to="`/member/order/${store.orderInfo.result.id}`"
            type="primary"
