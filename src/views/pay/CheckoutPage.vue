...
import type { SubmitOrderObject } from "@/types/Orders";
...
async function SubmitOrder() {
  const order: SubmitOrderObject = {
    ...
    addressId: receRef.value.getFinalAddress(),
    deliveryTimeType: 1,
    payType: 1,
    payChannel: 1,
    buyerMessage: "",
  ...
  try {
    ...
    message({ type: "success", msg: "成功" });
  } catch (err) {
    message({ type: "error", msg: "失败" });
  }
}
</script>
<template>
